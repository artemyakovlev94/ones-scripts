...
4. Если шаблон уже существует, сохраняет его как `1Cv8_backup_1.1CD`, а прежнюю первую копию — как `1Cv8_backup_2.1CD`.
5. Копирует исходный файл как `ONE_C_BASES_TEMPLATE\1Cv8.1CD`, заменяя существующий шаблон.

## Результат

В каталоге `ONE_C_BASES_TEMPLATE` находится актуальная копия файла базы и максимум две предыдущие версии: `1Cv8_backup_1.1CD` и `1Cv8_backup_2.1CD`. Исходная база не изменяется.
