...
Имя корневой логической папки в `%APPDATA%\1C\1CEStart\ibases.v8i`, например `DEV`. Это имя папки в списке запуска 1С, а не путь файловой системы. Не добавляйте начальный или конечный символ `/`. При удалении базы только из этой папки и её подпапок удаляется описание с совпадающим путём; другие папки не изменяются.
